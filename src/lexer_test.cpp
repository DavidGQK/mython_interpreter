...
void TestNumbers() {
    ...
    // Negative numbers are formed at the stage of syntactic analysis
    ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Char{'-'}));
...
x = 1
  ...
  z = 3
    ...
    ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Newline{}));
    // An empty string consisting only of whitespace characters does not change the current indent,
    // so the next token is Id, not Dedent
